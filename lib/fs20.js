 ...
 * @param address       string  device address - 2 digits hex string
...
module.exports.cmd = function (code, address, command, time, bidi, res) {
    ...
    // Text commands
    if (command.slice(0, 3) === 'dim' && command.slice(-1) !== '%') command += '%';
    if (commands.indexOf(command) !== -1) {
        command = ('0' + commands.indexOf(command).toString(16).toUpperCase()).slice(-2);
    }

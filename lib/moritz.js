...
module.exports.parse = function (raw) {
	...
	if ((2*message.len+3+2) != raw.length) { //+3 = +1 for 'Z' and +2 for len field in hex and +2 looks to be some checksum
		...
		addDeviceByMsgType(message.address, message.msgType);
		if (seen_devices[message.address]) message.device = seen_devices[message.address];
		if (seen_devices[message.dst]) message.dstDevice = seen_devices[message.dst];
